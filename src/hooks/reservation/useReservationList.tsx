...
export const useReservationList = () => {
  ...
  const { setReservationList, setIsReservationListLoading } = useTravelContext();
  const {
    userCred: { userId },
  } = useUserCredContext();
  ...
  useEffect(() => {
    setIsReservationListLoading(reservationListQuery.isLoading);
  }, [reservationListQuery.isLoading, setIsReservationListLoading]);

  return reservationListQuery;
};
