...
const saveAddNewDrivers = async ({ carListingId, hostId, fullName, email }: TSaveAddNewDrivers) => {
  const response = await axiosClient.post(`${apiUrl}/verify/send-driver-request-email/${hostId}`, {
    fullName,
    email,
    carListingId,
  });
...
export const useNotifyDrivers = () => {
  const { openSnackBar } = useSnackBarContext();
  const { setAdditionalDrivers, additionalDrivers } = useSearchContext(); //Set the additional Drivers
  return useMutation({
    mutationFn: ({ hostId, carListingId, fullName, email }: TSaveAddNewDrivers) => saveAddNewDrivers({ hostId, carListingId, fullName, email }),
    ...
    onError: (err: any) => {
      console.log('useNotifyDrivers mutation error', err);
      openSnackBar({
