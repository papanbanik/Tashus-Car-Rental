...
const ServicePhoto: React.FC<IFileUploadProps> = ({
  limit,
  multiple,
  name,
  control,
  ...
  useEffect(() => {
    if ((carData?.carServiceLog?.serviceLogs?.length ?? 0) > 0) {
      const { carServiceLog } = carData || {};
      const { serviceLogs } = carServiceLog || {};
      if (serviceLogs && serviceLogs.length > 0) {
        const lastIndex = serviceLogs.length - 1;
        const lastServiceLog = serviceLogs[lastIndex];
        const { documentInfo } = lastServiceLog;
        if (documentInfo?.info?.secure_url) {
          setSelectedProfileImage(documentInfo?.info?.secure_url);
        } else {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 md:gap-4">
        <SectionHeader title="Service Receipt" textSize="text-md" fontStyle="font-normal" noMargin={true}>
          {/* <span className="text-xs text-gray-600">{`  (optional)`}</span> */}
          <CommonTooltip
            title="Please provides a record of any maintenance, repairs, or inspections performed on your vehicle. Keeping service receipts is important for tax purposes, warranty claims, and maintaining your vehicle's value."
            placement="right"
            arrow
          >
     ...
     `}
            ...
            <div className="w-full h-8 absolute -bottom-2 left-1/2 transform -translate-x-1/2 z-10 rounded-full flex justify-center items-center gap-4">
              <Tooltip enterTouchDelay={0} title="Replace" placement="top">
                <IconButton size="large" className="bg-gray-400">
                  ...
                  <ImageUploader control={control} registerName="picture" onChangeFn={onFileDrop} multiple={false} required={false}></ImageUploader>
                </IconButton>
              </Tooltip>
            </div>
          </div>
          ...
          <Button variant="outlined" size="large" className="flex flex-col gap-2 justify-center items-center w-full h-[150px]">
            <FiUpload size={20} />
            <span>Upload</span>
            <ImageUploader control={control} registerName="picture" onChangeFn={onFileDrop} multiple={false} required={false} />
          </Button>
        )}
      </div>
