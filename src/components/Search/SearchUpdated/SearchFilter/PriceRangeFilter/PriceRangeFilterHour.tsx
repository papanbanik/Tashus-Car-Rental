...
const generateHistogramData = (availableCarList: TSearchedCar[]): Array<{ price: number; count: number; endPrice: number }> => {
  const maxPrice = Math.ceil(Math.max(...availableCarList.map((car) => car.rates.hourlyRates.amount || 0), 10));
  ...
  return Array.from({ length: totalBars }, (_, index) => {
    ...
    const count = availableCarList.filter((car) => {
      const hourlyRate = car.rates.hourlyRates.amount;
      return hourlyRate >= startPrice && (hourlyRate < endPrice || (index === totalBars - 1 && hourlyRate === maxPrice));
    }).length;
...
export function PriceRangeFilterHour({ initialRange, onChange, availableCarList }: PriceRangeFilterProps) {
  ...
  useEffect(() => {
    ...
    const filteredArray = availableCarList.filter((car) => {
      const hourlyRate = car.rates?.hourlyRates?.amount;
      return hourlyRate >= range.min && hourlyRate <= range.max;
    });
    ...
    <div className="w-full max-w-2xl">
      ...
      <RangeSlider
        ...
        initialRange={{
          ...
          max: Math.ceil(Math.max(...availableCarList.map((car) => car.rates.hourlyRates.amount || 0))),
        }}
