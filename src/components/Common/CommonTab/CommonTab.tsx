...
const CommonTab = ({ tabList, children, orientation, vehicleNickName, isSmall, isEdit }: CommonTabProps) => {
  ...
  const handleDropDownChange = (event: SelectChangeEvent<string>) => {
    ...
    if (selectedVehicle === 'seeAllVehicles') {
      router.push(`${process.env.NEXT_PUBLIC_DOMAIN}/profile/${userCred?.userId}/vehicles`);
    } else {
      ...
      if (selectedCar) {
        ...
        router.push(`${process.env.NEXT_PUBLIC_DOMAIN}/profile/${userCred?.userId}/vehicles/${selectedCar.listingId}/${tabList[value]?.routeName}`);
      } else {
        setListingId(selectedVehicle);
        router.push(`${process.env.NEXT_PUBLIC_DOMAIN}/profile/${userCred?.userId}/vehicles/${selectedVehicle}/${tabList[value]?.routeName}`);
      }
    }
  ...
  const handleChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    if (!orientation) {
      ...
      router.push(`${process.env.NEXT_PUBLIC_DOMAIN}/profile/${userCred?.userId}${routeString}`);
    } else {
      router.push(`${process.env.NEXT_PUBLIC_DOMAIN}/profile/${userCred?.userId}/vehicles/${listingId}/${tabList[newValue]?.routeName}`);
    }
  };
  ...
  return (
    <div
      className={`w-full  rounded ${
        orientation ? 'lg:h-[550px] lg:overflow-y-auto lg:sticky sm:top-28 bg-white mt-5 lg:mt-0' : 'flex-col'
          ...
          {tabList?.map((tab, index) => {
            ...
            return (
              <Tab
                ...
                label={
                  <span
                    className={`flex justify-start items-center w-[222px] ${value === index ? 'bg-primary text-white' : ''}`}
                  ...
                  >
                    <span
                      className={`text-3xl mt-1 me-3 ${value === index && value !== 9 && value !== 5 ? 'vehicleEditIconsList' : ''}`}
                ...
                className="pb-0 w-max  flex  flex-row "
              />
            );
          })}
        </Tabs>
