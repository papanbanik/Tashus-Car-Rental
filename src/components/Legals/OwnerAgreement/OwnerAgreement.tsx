...
const OwnerAgreement = () => {
  ...
  const updatedAgreement: PrivacyPolicy[] = useMemo(() => {
    ...
    allLegals.forEach((item: any) => {
      item.pageContents.forEach((item2: any) => {
        if (item2.isActive === true && item.pageIdentifier === '/legals/owner-agreement') {
          policyData.push({
  ...
  const reservationData: any = useQuery({
  ...
  useEffect(() => {
    if (reservationData?.status === 'success') {
      ...
      console.log('res', reservationData?.data?.data?.data);
      ...
      if (reservationData?.data?.data?.data?.vehicleInsurance) {
        setInsurance({
          ...reservationData?.data?.data?.data?.vehicleInsurance,
      ...
      <div className="bg-white w-full mb-10 px-5 md:px-10 py-3 md:py-6 col-span-6 md:col-span-4">
        <div ref={ref}>
          <h2 className="mb-10 text-center">
            Owner Agreement<span className="text-success">_</span>
          </h2>
            ...
            <>
              {!searchParams?.get('reservation-id') ? undefined : (
                <>
                  ...
                  {insurance?.coverageType ? (
                    <div className="mb-6">
                      <div className="font-bold text-xl mb-4">
                        <div className="font-bold mr-1 inline-block text-center ">{(count = count + 1)}.</div> Vehicle Insurance Coverage :{' '}
                      </div>
                      <div className="pl-4">
                        <div className="mb-1">
                          {' '}
                          <span className="font-semibold">
                            Coverage Type : {insurance?.coverageType ? capitalizeFirstLetter(insurance?.coverageType) : ''}
                          </span>
                        </div>
                        <div className="mb-1">
              ...
              {/* {!searchParams?.get('reservation-id') ? (
                <div className="text-base mt-20">
                  {' '}
                  <DisplayRichText content={`${updatedAgreement[0]?.pageContent || ''}`} />{' '}
                </div>
              ) : (
                <div className="text-base mt-20">
                  {' '}
                  <DisplayRichText content={`${reservationDetails?.agreement || ''}`} />{' '}
                </div>
              )} */}
