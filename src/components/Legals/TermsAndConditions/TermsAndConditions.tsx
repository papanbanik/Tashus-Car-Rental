...
import './PrivacyAndTermsCondition.css';
...
const PrivacyPage: React.FC = () => {
  ...
  const latestTermsAndCondition: PrivacyPolicy[] = useMemo(() => {
    if (!allLegals) return [];

    const policyData: PrivacyPolicy[] = [];
    allLegals.forEach((item: any) => {
      item.pageContents.forEach((item2: any) => {
        if (item2.isActive === true && item.pageIdentifier === '/legals/terms-and-conditions') {
          policyData.push({
  ...
  return (
    <Container maxWidth="lg">
      {isLoading ? (
        ...
        <>
          {latestTermsAndCondition &&
            latestTermsAndCondition?.map((item: any, index: number) => (
              <Grid className="mt-4 mb-4" container spacing={2} key={index}>
                <Grid item xs={12} md={8}>
                  <Paper style={{ padding: '20px', backgroundColor: 'transparent' }}>
                    ...
                    <Typography variant="body1" className="mb-8">
                      Updated on {formatFullDateTime(item?.updatedTime)}
                    </Typography>
                    <span className="text-base">
                      <DisplayRichText content={`${item?.pageContent || ''}`} />
                    </span>
                  </Paper>
                </Grid>
                ...
                <Grid item xs={12} md={4}>
                  <Paper style={{ padding: '20px', backgroundColor: 'transparent' }}>
                    ...
                    {item?.pageLinks &&
                      item.pageLinks.map((item2: any, index: number) => (
                        <List key={index}>
                          <ListItem>
                            <Link className="no-underline text-primary" href={`${item2?.linkUrl || '/'}`}>
                              <Typography>{`${item2?.linkTitle || ''}`}</Typography>
                            </Link>
                          </ListItem>
                        </List>
                      ))}
