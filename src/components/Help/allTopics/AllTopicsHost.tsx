...
const AllTopicsHost = ({ hostData, hostParentData }: { hostData: any; hostParentData: any }) => {
  return (
    <>
      <div className="mt-16">
        <Grid container spacing={4} className="block sm:flex">
          {hostParentData &&
            hostParentData.map((item: any, index: number) => (
              <Grid item key={index} xs={12} md={4}>
                <div className=" pl-0 sm:pl-5">
                  <div className="font-bold text-xl mb-2">{item.title ? item.title : ''}</div>
                  <div className="flex flex-col">
                    {hostData?.map((item2: any, index: number) =>
