...
const HostInfo = ({ hostArticleData }: { hostArticleData: any }) => {
  const firstTwelveHostArticles = hostArticleData && hostArticleData.slice(0, 12);
  return (
    <>
      <div className="mt-16">
        ...
        <Grid container spacing={3} className="block sm:flex">
          {firstTwelveHostArticles &&
            firstTwelveHostArticles?.map((item: any, index: number) => (
              <Grid item key={index} xs={12} md={4}>
                <div>
                  <div className="flex justify-between">
                    <Typography variant="h5" component="div">
                      <Link className="no-underline text-primary" href={`/help/article/${item.articleId}`}>
                        <Typography className="text-base md:text-xl underline underline-offset-1 " variant="h5" component="div">
                          {item.title}
                        </Typography>
                      </Link>
