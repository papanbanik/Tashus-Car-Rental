...
export const verifyMinTravelDays = async (
  ...
  if (unit === 'weeks') {
    const convertedDays = dayjs.duration({ weeks: shortestDuration }).as('days');
    return timeDiffDays >= convertedDays;
  }
};

export const verifyMaxTravelDays = async (
  unit: string,
  ...
  // if (unit === 'hours') {
  //   return timeDiffHours < longestDuration;
  // }

  if (unit === 'days') {
    return timeDiffDays <= longestDuration;
  }

  if (unit === 'weeks') {
    const convertedDays = dayjs.duration({ weeks: longestDuration }).as('days');
    return timeDiffDays <= convertedDays;
  }
};
...
export const validateSinglePickupReturn = (selectedDayList: any, selectedDateTimeData: any) => {
  ...
  if (selectedDayAvailability?.availability === 'custom') {
    ...
    const isBetweenFreeHours = selectedDayAvailability?.customHours?.some(
      (hours: any) => hours.status === 'free' && isTimeInBetween(hours.startTime, hours.endTime, selectedDateTimeData?.dateTime)
    );
...
export const getUpdatedReservationDates = async (reservationList: any) => {
  // console.log(reservationList);
  const updatedList = reservationList?.map((reservation: any) => {
    ...
    const lastRevision = revisedReservations?.slice(-1)?.[0];
    return {
...
export const validateBlockDates = async (blockDatesList: TCarBlockDate[], pickupDateTime: TDate, returnDateTime: TDate) => {
  const futureDates = blockDatesList?.filter((blockDate) => !dayjs(blockDate?.start).isBefore(dayjs(pickupDateTime), 'day'));
  // console.log(futureDates);
  let overlappedDate: TCarBlockDate = {} as TCarBlockDate;
  const isCarBlocked = futureDates?.some((blockDate) => {
    const isPickupBetween = dayjs(pickupDateTime).isBetween(dayjs(blockDate?.start), dayjs(blockDate?.end), 'minute', '[]');
    const isReturnBetween = dayjs(returnDateTime).isBetween(dayjs(blockDate?.start), dayjs(blockDate?.end), 'minute', '[]');
    const isBlockStartBetween = dayjs(blockDate?.start).isBetween(dayjs(pickupDateTime), dayjs(returnDateTime), 'minute', '[]');
    const isBlockEndBetween = dayjs(blockDate?.end).isBetween(dayjs(pickupDateTime), dayjs(returnDateTime), 'minute', '[]');
